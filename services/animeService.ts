...
export const searchAnime = async (query: string): Promise<Anime[]> => {
  try {
    ...
    if (!safeQuery) return [];
    const response = await fetch(`${JIKAN_URL}/anime?q=${encodeURIComponent(safeQuery)}&sfw=true&limit=24`);
...
export const fetchStreamSource = async (
  ...
  try {
    ...
    const sanitizedSources = data.sources
      .map((s: any) => {
        const safeUrl = sanitizeUrl(s.url);
        if (!safeUrl) return null;
        return {
          url: safeUrl,
          isM3U8: s.isM3U8 ?? safeUrl.includes('.m3u8'),
          quality: s.quality || s.resolution,
        };
      })
      .filter(Boolean);

    const sanitizedSubtitles = data.subtitles
      ?.map((sub: any) => {
        const safeUrl = sanitizeUrl(sub.url);
        if (!safeUrl) return null;
        return { url: safeUrl, lang: sub.lang || sub.label };
      })
      .filter(Boolean);

    if (sanitizedSources.length === 0) return null;
